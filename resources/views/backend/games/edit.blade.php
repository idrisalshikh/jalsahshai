...
<div class="container mx-auto p-4 max-w-4xl">
    <div class="mb-4">
        ...
        <p class="text-gray-600 text-sm">Edit the selected quiz game with questions.</p>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow-md">
        <form action="{{ route('admin.games.update', $game->id) }}" method="POST" enctype="multipart/form-data" id="game-form">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2 space-y-1">
                        ...
                        <textarea name="description"
                                  ...
                                  placeholder="Brief quiz description">{{ old('description', $game->description) }}</textarea>
                    ...
                    <div class="space-y-1">
                        ...
                        <div id="thumbnail-preview" class="mt-1 {{ $game->thumbnail ? '' : 'hidden' }}">
                            <img id="thumbnail-image" class="max-w-24 max-h-16 object-cover rounded border border-gray-300" src="{{ $game->thumbnail ? Storage::url($game->thumbnail) : '' }}" alt="Current game thumbnail">
                            <p class="text-xs text-gray-500">Current image - upload to replace</p>
                        </div>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   id="is_timed"
                                   value="1"
                                   {{ old('is_timed', $game->is_timed) ? 'checked' : '' }}
            ...
            <div class="mb-4">
                <h2 class="text-lg font-semibold mb-3 flex items-center">
                    ...
                    <span class="ml-auto text-sm text-gray-500">
                        Questions: <span id="questions-count" class="font-medium">{{ $game->questions->count() }}</span>
                    </span>
                </h2>
                ...
                <div id="questions-container" class="space-y-3">
                    ...
                    @else
                        @foreach($game->questions as $index => $question)
                            <div class="question-item border border-gray-200 rounded p-3 bg-gray-50" data-question-index="{{ $index }}">
                                <input type="hidden" name="questions[{{ $index }}][id]" value="{{ $question->id }}">
                                <div class="flex justify-between items-start mb-3">
                                    <h4 class="font-medium text-sm">Question {{ $index + 1 }}</h4>
                                    <button type="button" class="remove-question text-red-500 hover:text-red-700 p-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                                    ...
                                    <div class="lg:col-span-2 space-y-1">
                                        ...
                                        <div class="question-thumbnail-preview-{{ $index }} mt-1 {{ $question->thumbnail ? '' : 'hidden' }}">
                                            <img class="question-thumbnail-image-{{ $index }} max-w-24 max-h-16 object-cover rounded border border-gray-300" src="{{ $question->thumbnail ? Storage::url($question->thumbnail) : '' }}" alt="Question {{ $index + 1 }} thumbnail">
                                            <p class="text-xs text-gray-500">Current image - upload to replace</p>
                                        </div>
                                        <p class="text-xs text-gray-500">Optional image (max 2MB)</p>
                                    </div>
                                            ...
                                            <div class="space-y-2">
                                                ...
                                                @for($i = 0; $i < 4; $i++)
                                                    @php
                                                        ...
                                                        $isChecked = $question->correct_answer == $i ? 'checked' : '';
                                                    @endphp
                                                    <div class="flex items-center gap-2">
                                                        <input type="radio"
                                                               name="questions[{{ $index }}][correct_answer]"
                                                               value="{{ $i }}"
                                                               class="option-radio-{{ $i }} form-radio text-blue-600 h-4 w-4 {{ !$optionValue ? 'cursor-not-allowed' : '' }}"
                                                               data-option-index="{{ $i }}"
                                                               {{ $isChecked }}
                                                               required
                                                ...
                                                @endfor
                                                <p class="text-xs text-gray-500 mt-1">Select correct answer with radio button</p>
                                            </div>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex gap-3 w-full sm:w-auto">
                        ...
                        <button type="submit" id="save-game"
                            ...
                            <span class="save-text">Update Game</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let questionIndex = {{ count(old('questions', $game->questions)) }};
        ...
        gameForm.addEventListener('submit', function (e) {
            ...
            saveText.textContent = 'Updating Game...';
