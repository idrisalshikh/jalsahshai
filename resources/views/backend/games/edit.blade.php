...
<div class="container mx-auto p-8">
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Edit the selected quiz game with questions.</p>
    </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div id="thumbnail-preview" class="mt-2 {{ $game->thumbnail ? '' : 'hidden' }}">
                            <img id="thumbnail-image" class="max-w-xs max-h-32 object-cover rounded-md border border-gray-300" src="{{ $game->thumbnail ? Storage::url($game->thumbnail) : '' }}" alt="Current game thumbnail">
                            <p class="text-xs text-gray-500 mt-1">Current thumbnail - upload new image to replace</p>
                        </div>
                        @error('thumbnail')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div id="questions-container" class="space-y-4">
                    ...
                    @else
                        @foreach($game->questions as $index => $question)
                            <div class="question-item border border-gray-200 rounded-lg p-4 bg-gray-50" data-question-index="{{ $index }}">
                                <input type="hidden" name="questions[{{ $index }}][id]" value="{{ $question->id }}">
                                <div class="flex justify-between items-start mb-4">
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    <div class="lg:col-span-2">
                                        ...
                                        <div class="question-thumbnail-preview-{{ $index }} mt-2 {{ $question->thumbnail ? '' : 'hidden' }}">
                                            <img class="question-thumbnail-image-{{ $index }} max-w-xs max-h-32 object-cover rounded-md border border-gray-300" src="{{ $question->thumbnail ? Storage::url($question->thumbnail) : '' }}" alt="Question {{ $index + 1 }} thumbnail">
                                            <p class="text-xs text-gray-500 mt-1">Current thumbnail - upload new image to replace</p>
                                        </div>
                                            ...
                                            <div class="space-y-3">
                                                ...
                                                @for($i = 0; $i < 4; $i++)
                                                    @php
                                                        ...
                                                        $isChecked = $question->correct_answer === $optionValue && $optionValue ? 'checked' : '';
                                                    @endphp
                                                    <div class="flex items-center gap-3">
                                                        <input type="radio"
                                                               name="questions[{{ $index }}][correct_answer]"
                                                               value="{{ $optionValue ?: "option_{$i}" }}"
                                                               class="option-radio-{{ $i }} form-radio text-blue-600 {{ !$optionValue ? 'cursor-not-allowed' : '' }}"
                                                               data-option-index="{{ $i }}"
                                                               {{ $isChecked }}
                                                               required
                                                ...
                                                @endfor
                                                <p class="text-xs text-gray-500 mt-2">Select the correct answer by clicking the radio button next to it.</p>
                                            </div>
