...
<head>
    ...
    <title>Edit Session - Jalsah</title>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.games.update', $game->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" value="{{ $game->name }}" required>
            </div>
            <div class="mb-4">
                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">{{ $game->description }}</textarea>
            </div>
            <div class="mb-4">
                ...
                @foreach ($session->questions as $index => $question)
                    <div class="border p-4 rounded-md mb-4">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" name="questions[{{ $index }}][text]" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" value="{{ $question->text }}" required>
                        </div>
                        <div class="mb-2">
                            <label class="block text-sm font-medium text-gray-700">Type</label>
                            <select name="questions[{{ $index }}][type]" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm question-type">
                        ...
                        <div class="mb-2 options-container" style="{{ $question->type == 'true_false' ? 'display: none;' : '' }}">
                            <label class="block text-sm font-medium text-gray-700">Options (comma-separated)</label>
                            <input type="text" name="questions[{{ $index }}][options]" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" value="{{ is_array($question->options) ? implode(',', $question->options) : '' }}">
                        </div>
                        <div class="mb-2">
                            <label class="block text-sm font-medium text-gray-700">Correct Answer</label>
                            <input type="text" name="questions[{{ $index }}][correct_answer]" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" value="{{ $question->correct_answer }}" required>
